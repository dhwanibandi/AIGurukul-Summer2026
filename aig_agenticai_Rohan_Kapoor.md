...
- LLMs started from NLP research. Inflection point: 2017 paper *"Attention Is All You Need"* (transformers, Google) → then BERT → then GPT series.
...
- Architecture types:
  - **Encoder-only** (BERT) → good for understanding tasks (sentiment, classification, summarization).
  - **Decoder-only** (GPT, LLaMA) → good for generating text. **Most chatbots are decoder-only.**
  - **Encoder-decoder** (BART - Bidirectional and Auto-Regressive Transformers) → good for translation.
...
- **RAG** = keep a general model, supply relevant external text as context at query time.
- **Fine-tuning** = actually retrain/update the model's parameters on specific data (expensive, needs GPUs, like a person internalizing a subject over years).
- Findings: **RAG won on knowledge & conciseness**, **fine-tuning won on conversational style**. Fine-tuning needs lots of data to shine.
...
1. **Text → tokens**: tokenizer splits text using a fixed vocabulary (~60k tokens). Words not in vocab get broken into sub-pieces (e.g. "punctuation" → "punct" + "uation").
2. **Tokens → token numbers**: each token replaced by its index/serial number in the vocabulary.
3. **Token numbers → vectors**: mapped into N-dimensional space where similar/related words land close together.
- The **embeddings model is separate from the LLM**.

**Indexing pipeline (offline prep)**

- Source data (CSV, HTML, Word, PDF…) → extract text → embeddings model → vectors → store in **vector DB**.

**Query pipeline (at question time)**

1. User query → embeddings model → **query vector**.
...
- **Full fine-tuning**: updates **all** parameters → must store a copy of old weights too → very time/resource-intensive, higher forgetting risk.
- **LoRA / Parameter-Efficient Fine-Tuning**: updates only a **tiny fraction** of parameters (often **<1%**) → RAM/compute drops ~80%.
...
- **Use LoRA** for efficiency with large corpora, full fine-tuning only when you can afford it and manage forgetting.
...
**Build steps**: install libs → import → set API keys (Google account, Gemini/LLM key, CrewAI key) → define agents → define tasks → set `process = sequential` (or parallel).

**Chaining = the whole point**

- You can have **N agents / N tasks** in sequence: research → write → **review** → rewrite-based-on-review → **send email**.
...
- **RAG (Lecture 8)** = single agent, single task, info comes from your **vector DB** (e.g. Gita PDF). It's injected into the prompt, not preloaded into the model.
- **Multi-agent (here)** = info comes from a live **Google search**, passed to the writer task, the LLM summarizes it. The whole workflow re-runs for every query.
